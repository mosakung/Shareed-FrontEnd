...
httpGET('http://localhost:3000/shareed/share-event', urlParams.get("page"), (state, json) => {
    json.data.forEach(function (data) {
        $('.fetch-all-share-event').append(fourmCard(data));
    });
    ...
    $('.fetch-page-share-event').append(patternPage);
   ...
   json.tag.forEach(function (tempArray) {
    ...
    $('.fetch-tag-share-event').append(patternTag);
})
        ...
        <div class="sm-block" style="height:150px; padding: 5px;">
            <div class="row">
                ...
                <div class="col-sm-9" style="background-color: whitesmoke; width:390px;height: 140px; padding: 15px;">
                    <p style="color: red;">Share Event</p>
                    <p>${data.Title}</p>
                    <p>Day: ${(data.Data_Time).substring(0,10)} By: ${data.Username}</p>
                </div>
            </div> 
        </div>
    </a>
